...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	for (int i = 0; i < MAX_SOUND_BITES; i++)
	{
		if (soundBites[i].pSound == NULL)
		{
			...
			if (soundBites[i].pSound == NULL)
				printf("still null after assigning\n");
			pthread_mutex_unlock(&audioMutex);
			return;
		}
	}
	pthread_mutex_unlock(&audioMutex);
	printf("AudioMixer queue sound couldn't find free slots in soundBites\n");
	/*
...
void AudioMixer_cleanup(void)
{
	...
	if (temp == 0)
	{
		printf("soundBites is all NULL\n");
	}
	else
	{
		printf("soundBites was not NULL\n");
	}
...
void AudioMixer_setVolume(int newVolume)
{
	...
	const char *selem_name = "Master"; // For USB Audio  NOTE: this was changed from "Speaker"
									   //		const char *selem_name = "Speaker"; //Needed for others
...
static void fillPlaybackBuffer(short *playbackBuffer, int size)
{
	...
	memset(playbackBuffer, 0, size * sizeof(short)); //*playbackBuffer));
	// printf("After readWave test data: %d\n", *soundBites[0].pSound->pData);
	pthread_mutex_lock(&audioMutex);
	for (int i = 0; i < MAX_SOUND_BITES; i++)
	{
		if (soundBites[i].pSound != NULL)
		{
			...
			int totalSamples = sound.pSound->numSamples;
			int curLoc = sound.location;
			// printf("Samples: %d and Loc: %d\n", totalSamples, curLoc);
			for (int j = 0; j < size; j++, curLoc++)
			{
				// printf("Samples: %d and Loc: %d\n", totalSamples, curLoc);
				short curBuff = sound.pSound->pData[curLoc];
				if (playbackBuffer[j] > 0 && curBuff > SHRT_MAX - playbackBuffer[j])
				{
					playbackBuffer[j] = SHRT_MAX;
				}
				else if (playbackBuffer[j] < 0 && curBuff < SHRT_MIN + playbackBuffer[j])
				{
					playbackBuffer[j] = SHRT_MIN;
				}
				else
				{
					playbackBuffer[j] += curBuff;
				}
				// curLoc++;

				if (curLoc >= totalSamples)
				{
					soundBites[i].pSound = NULL;
					curLoc = 0;
					break;
...
void AudioMixer_printStats()
{
	sleep(1);

	Interval_statistics_t playbackStats;
	Interval_statistics_t beatStats;
	Interval_getStatisticsAndClear(INTERVAL_LOW_LEVEL_AUDIO, &playbackStats);
	Interval_getStatisticsAndClear(INTERVAL_BEAT_BOX, &beatStats);

	printf("Low[%.2f, %.2f] avg %.2f/%d", playbackStats.minIntervalInMs, playbackStats.maxIntervalInMs,
		   playbackStats.avgIntervalInMs, playbackStats.numSamples);
	printf("Beat[%.2f, %.2f] avg %.2f/%d", beatStats.minIntervalInMs, beatStats.maxIntervalInMs,
		   beatStats.avgIntervalInMs, beatStats.numSamples);
}
